...
export function CSVImport({ onImport, onCancel, existingPilots = [] }: CSVImportProps) {
  ...
  return (
    <div className="fixed inset-0 bg-void/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-night border-2 border-steel rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="border-b border-steel p-6 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-chrome font-['Space_Grotesk']">
            CSV Import
          </h2>
        <button
          onClick={onCancel}
          className="text-steel hover:text-neon-cyan transition-colors text-2xl"
        >
          ×
        </button>
        </div>
        ...
        <div className="p-6">
          {!state.result ? (
            ...
            <div className="space-y-6">
              ...
              {/* Actions */}
              <div className="flex justify-end gap-4 pt-4 border-t border-steel">
                <button
                  onClick={onCancel}
